#!/bin/sh
# This script is copied from neutron and adapted for ixfabric.
set -eu

usage () {
    echo "Usage: $0 [OPTION]..."
    echo "Run ixfabric's coding check(s)"
    echo ""
    echo "  -Y, --pylint [<basecommit>] Run pylint check on the entire ixfabric module or just files changed in basecommit (e.g. HEAD~1)"
