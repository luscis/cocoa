...
check_pot_files_errors () {
    find ixfabric -type f -regex '.*\.pot?' \
...
check_identical_policy_files () {
    ...
    # independent of whether it's executed from the ixfabric source tree or from
    # site-packages installation path. We don't want two copies of the same
    # file to diverge, so checking that they are identical
    diff etc/policy.yaml ixfabric/tests/etc/policy.yaml 2>&1 > /dev/null
    if [ "$?" -ne 0 ]; then
...
check_no_duplicate_api_test_idempotent_ids() {
    ...
    output=$(check-uuid --package ixfabric.tests.tempest)
    if [ "$?" -ne 0 ]; then
